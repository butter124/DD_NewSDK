#pragma once

// Dungeon Defenders (10.0) SDK

#ifdef _MSC_VER
	#pragma pack(push, 0x4)
#endif

namespace Classes
{
//---------------------------------------------------------------------------
//Classes
//---------------------------------------------------------------------------

// Class CDT7.DunDefProjectile_HarpoonDot
// 0x001C (0x03AC - 0x0390)
class ADunDefProjectile_HarpoonDot : public ADunDefProjectile_Harpoon
{
public:
	float                                              DotDamageScale;                                           // 0x0390(0x0004) (Edit)
	class ADunDefEmitterSpawnable*                     DotTemplate;                                              // 0x0394(0x0004) (Edit)
	unsigned long                                      bAttach : 1;                                              // 0x0398(0x0004) (Edit)
	unsigned long                                      bUseDot : 1;                                              // 0x0398(0x0004) (Edit)
	unsigned long                                      bSlowEnemyTarget : 1;                                     // 0x0398(0x0004) (Edit)
	unsigned long                                      bWeakenEnemyTarget : 1;                                   // 0x0398(0x0004) (Edit)
	float                                              SlowEnemyTargetPercentage;                                // 0x039C(0x0004) (Edit)
	float                                              WeakenEnemyTargetPercentage;                              // 0x03A0(0x0004) (Edit)
	float                                              EnemyClearSlowTime;                                       // 0x03A4(0x0004) (Edit)
	float                                              EnemyClearWeakenTime;                                     // 0x03A8(0x0004) (Edit)

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.DunDefProjectile_HarpoonDot");
		return ptr;
	}


	void ProcessTouch(class AActor* Other, const struct FVector& HitLocation, const struct FVector& HitNormal);
	void SpawnDot(const struct FVector& HitLocation, const struct FVector& HitNormal);
};


// Class CDT7.DunDefProjectile_StaffDot
// 0x0020 (0x03CC - 0x03AC)
class ADunDefProjectile_StaffDot : public ADunDefProjectile_MagicBolt
{
public:
	float                                              DotDamageScale;                                           // 0x03AC(0x0004) (Edit)
	class ADunDefEmitterSpawnable*                     DotTemplate;                                              // 0x03B0(0x0004) (Edit)
	class ADunDefEmitterSpawnable*                     EffectTemplate;                                           // 0x03B4(0x0004) (Edit)
	unsigned long                                      bAttach : 1;                                              // 0x03B8(0x0004) (Edit)
	unsigned long                                      bUseDot : 1;                                              // 0x03B8(0x0004) (Edit)
	unsigned long                                      bUseEffect : 1;                                           // 0x03B8(0x0004) (Edit)
	unsigned long                                      bWeakenEnemyTarget : 1;                                   // 0x03B8(0x0004) (Edit)
	unsigned long                                      bSlowEnemyTarget : 1;                                     // 0x03B8(0x0004) (Edit)
	unsigned long                                      bUseParticleEffectOnHit : 1;                              // 0x03B8(0x0004) (Edit)
	float                                              SlowEnemyTargetPercentage;                                // 0x03BC(0x0004) (Edit)
	float                                              WeakenEnemyTargetPercentage;                              // 0x03C0(0x0004) (Edit)
	float                                              EnemyClearSlowTime;                                       // 0x03C4(0x0004) (Edit)
	float                                              EnemyClearWeakenTime;                                     // 0x03C8(0x0004) (Edit)

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.DunDefProjectile_StaffDot");
		return ptr;
	}


	void SpawnEffect();
	void Landed(const struct FVector& HitNormal, class AActor* Floor);
	void SpawnDot(const struct FVector& HitLocation, const struct FVector& HitNormal);
	void DoEffect();
	void Explode(const struct FVector& HitLocation, const struct FVector& HitNormal);
};


// Class CDT7.DunDefWeapon_MagicStaff_Dot
// 0x0000 (0x0658 - 0x0658)
class ADunDefWeapon_MagicStaff_Dot : public ADunDefWeapon_MagicStaff
{
public:

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.DunDefWeapon_MagicStaff_Dot");
		return ptr;
	}


	void ShootProjectiles(int theFireMode);
};


// Class CDT7.HeroEquipment_Familiar_TowerDamageScaling
// 0x00FC (0x0CC4 - 0x0BC8)
class UHeroEquipment_Familiar_TowerDamageScaling : public UHeroEquipment_Familiar
{
public:
	float                                              ProjectileShootInterval;                                  // 0x0BC8(0x0004) (Edit)
	class ADunDefProjectile*                           ProjectileTemplate;                                       // 0x0BCC(0x0004) (Edit)
	class ADunDefProjectile*                           ProjectileTemplateAlt;                                    // 0x0BD0(0x0004) (Edit)
	TArray<class ADunDefProjectile*>                   ProjectileTemplates;                                      // 0x0BD4(0x000C) (Edit, NeedCtorLink)
	struct FName                                       FamiliarSocketName;                                       // 0x0BE0(0x0008) (Edit)
	struct FName                                       MuzzleSocketName;                                         // 0x0BE8(0x0008) (Edit)
	struct FName                                       ShootEffectSocket;                                        // 0x0BF0(0x0008) (Edit)
	float                                              TargetRange;                                              // 0x0BF8(0x0004) (Edit)
	unsigned long                                      ScaleTowerDamage : 1;                                     // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bUseFixedShootSpeed : 1;                                  // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bShootProjectileWithoutTarget : 1;                        // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bForceProjectileMuzzleRotation : 1;                       // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bIgnoreHurtAnimationWhenShooting : 1;                     // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bSlowEnemyTarget : 1;                                     // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bWeakenEnemyTarget : 1;                                   // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bUseProjectileImpactedDelegate : 1;                       // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bMythicalScaleTowerDamage : 1;                            // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bChooseHealingTarget : 1;                                 // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bProjectilesCollideWithOwner : 1;                         // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bAddManaForDamage : 1;                                    // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bUseAltProjectile : 1;                                    // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bIgnoreElementInTargeting : 1;                            // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bDoShotsPerSecondBonusCap : 1;                            // 0x0BFC(0x0004) (Edit)
	unsigned long                                      DoLineOfSightCheck : 1;                                   // 0x0BFC(0x0004) (Edit)
	unsigned long                                      bDidAddManaForDamage : 1;                                 // 0x0BFC(0x0004) (Transient)
	TArray<struct FVector>                             ProjectileSpawnOffsets;                                   // 0x0C00(0x000C) (Edit, NeedCtorLink)
	struct FVector                                     ProjectileSpawnOffset;                                    // 0x0C0C(0x000C) (Edit)
	TArray<struct FRotator>                            ProjectileRotOffsets;                                     // 0x0C18(0x000C) (Edit, NeedCtorLink)
	TArray<float>                                      ProjectileDelays;                                         // 0x0C24(0x000C) (Edit, NeedCtorLink)
	float                                              MinimumProjectileSpeed;                                   // 0x0C30(0x0004) (Edit)
	float                                              ProjectileSpeedBonusMultiplier;                           // 0x0C34(0x0004) (Edit)
	float                                              MaxAttackAnimationSpeed;                                  // 0x0C38(0x0004) (Edit)
	float                                              ProjectileDamageMultiplier;                               // 0x0C3C(0x0004) (Edit)
	float                                              ShotsPerSecondExponent;                                   // 0x0C40(0x0004) (Edit)
	float                                              ShotsPerSecondAnimExponent;                               // 0x0C44(0x0004) (Edit)
	float                                              AbsoluteDamageMultiplier;                                 // 0x0C48(0x0004) (Edit)
	float                                              SlowEnemyTargetPercentage;                                // 0x0C4C(0x0004) (Edit)
	float                                              WeakenEnemyTargetPercentage;                              // 0x0C50(0x0004) (Edit)
	float                                              EnemyClearSlowTime;                                       // 0x0C54(0x0004) (Edit)
	float                                              MythicalScaleDamageStatExponent;                          // 0x0C58(0x0004) (Edit)
	float                                              EnemyClearWeakenTime;                                     // 0x0C5C(0x0004) (Edit)
	float                                              NightmareDamageMultiplier;                                // 0x0C60(0x0004) (Edit)
	float                                              ExtraNightmareDamageMultiplier;                           // 0x0C64(0x0004) (Edit)
	TEnumAsByte<ELevelUpValueType>                     MythicalScaleDamageStatType;                              // 0x0C68(0x0001) (Edit)
	unsigned char                                      UnknownData00[0x3];                                       // 0x0C69(0x0003) MISSED OFFSET
	class USoundCue*                                   ShootSound;                                               // 0x0C6C(0x0004) (Edit)
	class UParticleSystem*                             ShootEffect;                                              // 0x0C70(0x0004) (Edit)
	float                                              BaseHealAmount;                                           // 0x0C74(0x0004) (Edit)
	float                                              HealAmountMultiplier;                                     // 0x0C78(0x0004) (Edit)
	float                                              NightmareHealingMultiplier;                               // 0x0C7C(0x0004) (Edit)
	float                                              AltProjectileMinimumRange;                                // 0x0C80(0x0004) (Edit)
	float                                              HealingPriorityHealthPercentage;                          // 0x0C84(0x0004) (Edit)
	struct FName                                       AttackAnimationAlt;                                       // 0x0C88(0x0008) (Edit)
	int                                                ShotsPerSecondBonusCap;                                   // 0x0C90(0x0004) (Edit)
	float                                              BaseDamageToManaRatio;                                    // 0x0C94(0x0004) (Edit)
	float                                              ManaMultiplier;                                           // 0x0C98(0x0004) (Edit)
	float                                              MaxManaPerDamage;                                         // 0x0C9C(0x0004) (Edit)
	float                                              MinManaPerDamage;                                         // 0x0CA0(0x0004) (Edit)
	float                                              MaxManaMultiplierExponent;                                // 0x0CA4(0x0004) (Edit)
	float                                              DamageManaMultiplierExponent;                             // 0x0CA8(0x0004) (Edit)
	float                                              LastShootProjectileTime;                                  // 0x0CAC(0x0004) (Transient)
	TScriptInterface<class UDunDefTargetableInterface> BestTargetRef;                                            // 0x0CB0(0x0008)
	TArray<struct UHeroEquipment_Familiar_TowerDamageScaling_FDelayedShot> DelayedShots;                                             // 0x0CB8(0x000C) (Transient, NeedCtorLink)

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.HeroEquipment_Familiar_TowerDamageScaling");
		return ptr;
	}


	float GetHealAmount();
	void OnDunDefProjectileImpacted(class ADunDefProjectile* aProjectile, class AActor* impactedOn);
	void NotifyEquipment_PawnTookDamage(class AController* InstigatedBy, const struct FVector& HitLocation, int Damage, class UClass* DamageType, const struct FVector& Momentum, class AActor* DamageCauser);
	void NotifyEquipment_AnimNotifyAttachment(int NotifyID, class USkeletalMeshComponent* equipmentSkelComp);
	void ShootProjectile(unsigned long bIsAltProjectile);
	void GetMuzzleTransformation(class USkeletalMeshComponent* myMeshComp, struct FVector* SpawnLocation, struct FRotator* SpawnRotation);
	int GetWeaponDamage();
	void ApplyPrimaryDamageMultiplierDisplay(int* StatValue);
	float GetEquipmentStatValue(TEnumAsByte<EEquipmentStatType> equipmentStatType, unsigned long returnFinalAddedValue, unsigned long includeStatLevelUp);
	void AddRandomizeValues(float equipmentQuality, unsigned long doResetStatsToTemplate, unsigned long bDontUseMissionRandomizerMultiplier, float RandomizerMultiplierOverride, unsigned long bIsForShop, unsigned long bAllowTranscendentGear);
	unsigned long AllowStatLevelUp(TEnumAsByte<EEquipmentStatType> EQS, int statSubIndex, unsigned long IsInForgeUI, unsigned long bIgnoreManaRequirement);
	float GetAttackInterval(unsigned long bIncludeShotsPerSecondExponent);
	class UClass* GetElementalDamageType();
	float GetProjectileElementalDamage();
	float GetProjectileDamage(class ADunDefProjectile* projectileArch);
	float GetProjectileSpeed(class ADunDefProjectile* projectileArch);
	int GetNumProjectiles();
	struct FRotator GetDesiredSphereRotationalPlacement(class USkeletalMeshComponent* myMeshComp, class ADunDefPlayer* myPlayer, unsigned char* HasTarget);
	struct FRotator GetDesiredDirection(class USkeletalMeshComponent* myMeshComp, class ADunDefPlayer* myPlayer);
	void ShutDown(unsigned long isDestruction);
	void AttachedComponent(class UPrimitiveComponent* aComp);
	void UpdateAI(class ADunDefPlayer* PlayerOwner, float DeltaTime);
	void UpdateDelayedShots(class ADunDefPlayer* PlayerOwner, float DeltaTime);
	void TickedByPawn(class ADunDefPlayer* PlayerOwner, float DeltaTime);
	TScriptInterface<class UDunDefTargetableInterface> GetBestTarget();
	TScriptInterface<class UDunDefTargetableInterface> GetBestHealingTarget(float CheckHealthPercentageThreshold);
};


// Class CDT7.HeroEquipment_Familiar_Melee_TowerScaling
// 0x0060 (0x0D24 - 0x0CC4)
class UHeroEquipment_Familiar_Melee_TowerScaling : public UHeroEquipment_Familiar_TowerDamageScaling
{
public:
	float                                              MeleeHitRadius;                                           // 0x0CC4(0x0004) (Edit)
	float                                              MeleeDamageMomentum;                                      // 0x0CC8(0x0004) (Edit)
	class UClass*                                      MeleeDamageType;                                          // 0x0CCC(0x0004) (Edit)
	float                                              MaxKnockbackMultiplier;                                   // 0x0CD0(0x0004) (Edit)
	float                                              KnockbackBonusLinearScale;                                // 0x0CD4(0x0004) (Edit)
	float                                              KnockbackBonusExpScale;                                   // 0x0CD8(0x0004) (Edit)
	unsigned long                                      bAlsoShootProjectile : 1;                                 // 0x0CDC(0x0004) (Edit)
	unsigned long                                      ScaleMeleeDamageForHero : 1;                              // 0x0CDC(0x0004) (Edit)
	unsigned long                                      bUseRandomizedDamage : 1;                                 // 0x0CDC(0x0004) (Edit)
	unsigned long                                      bDoMeleeHealing : 1;                                      // 0x0CDC(0x0004) (Edit)
	unsigned long                                      bIsAttacking : 1;                                         // 0x0CDC(0x0004)
	float                                              MeleeRange;                                               // 0x0CE0(0x0004) (Edit)
	float                                              ScaleDamageStatExponent;                                  // 0x0CE4(0x0004) (Edit)
	float                                              ExtraNightmareMeleeDamageMultiplier;                      // 0x0CE8(0x0004) (Edit)
	struct FName                                       AlsoShootProjectileAnimation;                             // 0x0CEC(0x0008) (Edit)
	TEnumAsByte<ELevelUpValueType>                     ScaleMeleeDamageForHeroStatType;                          // 0x0CF4(0x0001) (Edit)
	unsigned char                                      UnknownData00[0x3];                                       // 0x0CF5(0x0003) MISSED OFFSET
	int                                                RandomizedDamageMultiplierMaximum;                        // 0x0CF8(0x0004) (Edit)
	float                                              RandomizedDamageMultiplierDivisor;                        // 0x0CFC(0x0004) (Edit)
	float                                              BaseDamageToHealRatio;                                    // 0x0D00(0x0004) (Edit)
	float                                              EquipmentDamageMultiplierBase;                            // 0x0D04(0x0004) (Edit)
	float                                              MaxHealPerDamage;                                         // 0x0D08(0x0004) (Edit)
	float                                              MinHealPerDamage;                                         // 0x0D0C(0x0004) (Edit)
	float                                              MaxHealMultiplierExponent;                                // 0x0D10(0x0004) (Edit)
	float                                              DamageHealMultiplierExponent;                             // 0x0D14(0x0004) (Edit)
	TArray<class AActor*>                              SwingHurtList;                                            // 0x0D18(0x000C) (NeedCtorLink)

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.HeroEquipment_Familiar_Melee_TowerScaling");
		return ptr;
	}


	void AttachedComponent(class UPrimitiveComponent* aComp);
	void NotifyEquipment_AnimNotifyAttachment(int NotifyID, class USkeletalMeshComponent* equipmentSkelComp);
	void ShutDown(unsigned long isDestruction);
	void UpdateAI(class ADunDefPlayer* PlayerOwner, float DeltaTime);
	float GetHealMultiplier();
	float GetMomentumMultiplier();
	void UpdateAttackSwing(float DeltaTime);
	unsigned long AddToSwingHurtList(class ADunDefPlayer* myOwner, class AActor* newEntry);
	void TickedByPawn(class ADunDefPlayer* PlayerOwner, float DeltaTime);
};


// Class CDT7.UI_TBR
// 0x0134 (0x0770 - 0x063C)
class UUI_TBR : public UDunDefUIScene
{
public:
	class UUIButton*                                   CloseButton;                                              // 0x063C(0x0004) (Edit)
	TArray<class UUIScriptWidget_Button*>              CheckPointButtons;                                        // 0x0640(0x000C) (Edit, NeedCtorLink)
	struct FOptionsInfo                                SavedOptions;                                             // 0x064C(0x0104) (NeedCtorLink)
	class UDunDefHeroManager*                          heroManager;                                              // 0x0750(0x0004)
	struct FString                                     MapToLaunch;                                              // 0x0754(0x000C) (NeedCtorLink)
	int                                                MapSelection;                                             // 0x0760(0x0004)
	int                                                DifficultySelection;                                      // 0x0764(0x0004)
	class UUIScriptWidget_Button*                      TBRAscensionStartButton;                                  // 0x0768(0x0004) (Edit)
	class UUIScriptWidget_Button*                      TBRNightmareStartButton;                                  // 0x076C(0x0004) (Edit)

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.UI_TBR");
		return ptr;
	}


	void LaunchMap(unsigned long bStartTrueBossRush);
	unsigned long WasCheckPointButtonPressed(class UUIObject* Widget);
	unsigned long NotifyWidgetClicked(class UUIObject* Widget);
	void CheckProgress();
	void SceneActivated(unsigned long bInitialActivation);
};


// Class CDT7.UI_ShopBase
// 0x009C (0x06D8 - 0x063C)
class UUI_ShopBase : public UDunDefUIScene
{
public:
	int                                                ShopID;                                                   // 0x063C(0x0004) (Edit)
	TArray<class UUIButton_Shop*>                      Btn_ShopButtons;                                          // 0x0640(0x000C) (Edit, NeedCtorLink)
	TArray<class UUILabel*>                            Lb_CurrencyLabels;                                        // 0x064C(0x000C) (Edit, NeedCtorLink)
	class UUIPanel_DataList*                           ItemDataList;                                             // 0x0658(0x0004) (Edit)
	unsigned long                                      bUseDynamicDataList : 1;                                  // 0x065C(0x0004) (Edit)
	unsigned long                                      bUseKeyModifers : 1;                                      // 0x065C(0x0004) (Edit)
	unsigned long                                      bFireKismetEventOnPurchase : 1;                           // 0x065C(0x0004) (Edit)
	struct FsShopInfo                                  ShopInfo;                                                 // 0x0660(0x0038) (Edit, NeedCtorLink)
	TArray<struct FsShopInfo>                          ShopInfoPerTab;                                           // 0x0698(0x000C) (Edit, NeedCtorLink)
	class UUIButton*                                   CloseButton;                                              // 0x06A4(0x0004) (Edit)
	class UUILabel*                                    Lb_ShopName;                                              // 0x06A8(0x0004) (Edit)
	class UUILabel*                                    Lb_ShopDescription;                                       // 0x06AC(0x0004) (Edit)
	struct FName                                       KismetEventName;                                          // 0x06B0(0x0008) (Edit)
	class UDunDefUIScene*                              UI_ConfirmBoxTemplate;                                    // 0x06B8(0x0004) (Edit)
	class UDunDefHeroManager*                          heroManager;                                              // 0x06BC(0x0004) (Transient)
	int                                                UserID;                                                   // 0x06C0(0x0004) (Transient)
	class UDunDefHero*                                 currentHero;                                              // 0x06C4(0x0004) (Transient)
	class UUIButton_Shop*                              LastSelecteditem;                                         // 0x06C8(0x0004) (Transient)
	class UUIObject*                                   LastSelectedButton;                                       // 0x06CC(0x0004) (Transient)
	int                                                Amount;                                                   // 0x06D0(0x0004) (Transient)
	class ADunDefPlayerController*                     CurrentPlayerController;                                  // 0x06D4(0x0004) (Transient)

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.UI_ShopBase");
		return ptr;
	}


	class UUIButton_DataListEntry* GetSelectedShopButton();
	struct FsShopInfo GetShopInfo();
	struct FsShopItemInfo GetItemInfo(class UUIButton_Shop* Button, unsigned long bGetValueFromFocusControl, unsigned long bGetValueFromLastSelectedButton);
	struct FString GetShopName(unsigned long bGetDescription);
	unsigned long AllowOverlays();
	TArray<class UHeroEquipment*> GetConsumedItemList(const struct FsShopItemInfo& ShopItemInfo);
	class UUIButton_Shop* GetLastSelectedItem();
	void GiveMana(const struct FsShopItemInfo& ShopItemInfo, class ADunDefPlayerController* PlayerController);
	unsigned long GenerateItem(const struct FsShopItemInfo& ShopItemInfo);
	void PurchaseItem(const struct FsShopItemInfo& ShopItemInfo);
	void EntrySelected(class UUIButton_DataListEntry* Button);
	void RefreshedList();
	void RefreshItemList(int Id);
	unsigned long OnReceivedInputKey(struct FInputEventParameters* EventParms);
	void CancelClicked();
	void ConfirmClicked();
	void OpenError(TEnumAsByte<EErrorCode> ErrorCode);
	void OpenConfirm(const struct FsShopItemInfo& ItemInfo);
	void OnItemPurchased(class UUIObject* CallerObject);
	unsigned long NotifyWidgetClicked(class UUIObject* Widget);
	int GetTotalCost(const struct FsShopItemInfo& ShopItemInfo);
	void UpdateUI();
	unsigned long CanPurchaseItem(class UUIButton_Shop* Button);
	void ConsumeItems(const struct FsShopItemInfo& ShopItemInfo, TArray<class UHeroEquipment*> ItemsToConsume, const struct FString& ManaToConsume);
	void ClearCurrencyAmounts();
	struct FString FormatManaString(const struct FString& ManaStringin);
	void UpdateCurrencyAmounts();
	unsigned long InitializeShop();
	void PostInitialSceneUpdate();
	void SceneActivated(unsigned long bInitialActivation);
};


// Class CDT7.UI_CoalForge
// 0x0000 (0x06D8 - 0x06D8)
class UUI_CoalForge : public UUI_ShopBase
{
public:

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.UI_CoalForge");
		return ptr;
	}

};


// Class CDT7.UI_ConfirmBox
// 0x013C (0x0778 - 0x063C)
class UUI_ConfirmBox : public UDunDefUIScene
{
public:
	class UUIButton*                                   Lbt_ConfirmButton;                                        // 0x063C(0x0004) (Edit)
	class UUIButton*                                   Lbt_CancelButton;                                         // 0x0640(0x0004) (Edit)
	class UUIButton*                                   Lbt_ErrorConfirm;                                         // 0x0644(0x0004) (Edit)
	class UUILabel*                                    Lb_ConfirmText;                                           // 0x0648(0x0004) (Edit)
	class UUIPanel*                                    Pnl_ItemInfo;                                             // 0x064C(0x0004) (Edit)
	class UUILabel*                                    Lb_ItemDescription;                                       // 0x0650(0x0004) (Edit)
	TArray<class UUILabel*>                            CurrencyLabels;                                           // 0x0654(0x000C) (Edit, NeedCtorLink)
	class UUIImage*                                    ItemIcon;                                                 // 0x0660(0x0004) (Edit)
	TArray<class UUIImage*>                            CurrencyIcons;                                            // 0x0664(0x000C) (Edit, NeedCtorLink)
	unsigned long                                      bShopInitalized : 1;                                      // 0x0670(0x0004) (Transient)
	struct FsShopInfo                                  CachedShopInfo;                                           // 0x0674(0x0038) (Transient, NeedCtorLink)
	struct FsShopItemInfo                              CachedShopItemInfo;                                       // 0x06AC(0x00B4) (Transient, NeedCtorLink)
	struct FScriptDelegate                             __OnConfirmClicked__Delegate;                             // 0x0760(0x000C) (Transient, NeedCtorLink)
	struct FScriptDelegate                             __OnCancelClicked__Delegate;                              // 0x076C(0x000C) (Transient, NeedCtorLink)

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.UI_ConfirmBox");
		return ptr;
	}


	unsigned long OnReceivedInputKey(struct FInputEventParameters* EventParms);
	void PostInitialSceneUpdate();
	unsigned long OnButtonPressed(class UUIScreenObject* EventObject, int PlayerIndex);
	void InitalizeItemFromShop(const struct FsShopItemInfo& ItemInfo, const struct FsShopInfo& ShopInfo);
	void SceneActivated(unsigned long bInitialActivation);
	void OnCancelClicked();
	void OnConfirmClicked();
};


// Class CDT7.UIButton_Shop
// 0x00E0 (0x08D8 - 0x07F8)
class UUIButton_Shop : public UUIScriptWidget_EquipmentIconButton
{
public:
	struct FsShopItemInfo                              ItemInfo;                                                 // 0x07F8(0x00B4) (Edit, NeedCtorLink)
	unsigned long                                      bCanRefreshItemTemplate : 1;                              // 0x08AC(0x0004) (Edit)
	float                                              TooltipItemIconPaddingX;                                  // 0x08B0(0x0004) (Edit)
	float                                              TooltipItemIconPaddingY;                                  // 0x08B4(0x0004) (Edit)
	float                                              TooltipCurrencyTextPaddingX;                              // 0x08B8(0x0004) (Edit)
	float                                              TooltipCurrencyTextPaddingY;                              // 0x08BC(0x0004) (Edit)
	float                                              TooltipItemIconScaleX;                                    // 0x08C0(0x0004) (Edit)
	float                                              TooltipItemIconScaleY;                                    // 0x08C4(0x0004) (Edit)
	float                                              SpaceBetweenCurrencyEntries;                              // 0x08C8(0x0004) (Edit)
	struct FScriptDelegate                             __NotifyWidgetClicked__Delegate;                          // 0x08CC(0x000C) (NeedCtorLink)

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.UIButton_Shop");
		return ptr;
	}


	void DrawToolTip(class UCanvas* C, float X1, float X2, float Y1, float Y2, float ScaleX, float ScaleY);
	void RenderGame(class UCanvas* C, float X1, float X2, float Y1, float Y2);
	class UHeroEquipment* GetEquipment();
	void UpdateItemInfo();
	class UUIDataContainer_Equipment* GetDataContainer();
	struct FString GetToolTipString();
	void InitializeFromDataListEntry(class UUIPanel_DataList* DataList, const TScriptInterface<class UDataListEntryInterface>& Entry);
	struct FsShopInfo GetShopInfo();
	struct FsShopItemInfo GetShopItemInfo();
	unsigned long ButtonClicked(class UUIScreenObject* Sender, int PlayerIndex);
	unsigned long NotifyWidgetClicked(class UUIObject* Widget);
};


// Class CDT7.IShopHandler
// 0x0000 (0x003C - 0x003C)
class UIShopHandler : public UInterface
{
public:

	static UClass* StaticClass()
	{
		static auto ptr = UObject::FindClass("Class CDT7.IShopHandler");
		return ptr;
	}


	void OnItemPurchased(class UUIObject* CallerObject);
};


}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
